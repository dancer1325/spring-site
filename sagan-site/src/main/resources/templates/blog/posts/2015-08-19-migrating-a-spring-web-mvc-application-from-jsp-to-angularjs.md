---
title: Migrating a Spring Web MVC application from JSP to AngularJS
source: https://spring.io/blog/2015/08/19/migrating-a-spring-web-mvc-application-from-jsp-to-angularjs
scraped: 2026-02-23T19:44:16.507Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Michael Isvy |  August 19, 2015 | 33 Comments
---

# Migrating a Spring Web MVC application from JSP to AngularJS

_Engineering | Michael Isvy |  August 19, 2015 | 33 Comments_

# [](#note-on-authors)Note on authors
...
## [](#sample-petclinic-for-reference)Sample Petclinic for reference
...
```
Copycomponents/ # the various components are stored here.
...
```html
Copy<form:form method="POST" commandName="user">
<table>
    <tr>
        <td>User Name :</td>
...
```html
Copy<form name="UserForm" data-ng-controller="ExampleUserController">
  <table>
    <tr>
        <td>User Name :</td>
...
# [](#considerations-when-moving-from-jsp-to-angularjs)Considerations when moving from JSP to AngularJS
...
## [](#converting-your-spring-controllers-to-restful-services)Converting your Spring controllers to RESTful services
...
```java
Copy@RequestMapping("/api/owners/{ownerId}")
...
```java
Copy@RequestMapping(value = "/api/owners/{id}", method = RequestMethod.GET)
...
```xml
Copy<bean id="objectMapper" class="org.springframework.http.converter.json.Jackson2ObjectMapperFactoryBean" p:indentOutput="true" p:simpleDateFormat="yyyy-MM-dd'T'HH:mm:ss.SSSZ"></bean>
...
```javascript
Copyvar Owner = ['$resource','context', function($resource, context) {
...
When moving to RestAPI, it is important to remember that the RestAPI is the public interface rather than the website content. The JSON model is **fully visible** to users. For example, if we need to display user profiles, password masking should be done on the JSON object rather than in the template. In order to do this, sometimes we need to create DTO objects for our RestAPI.

## [](#synchronizing-states-between-the-backend-and-your-angularjs-application)Synchronizing states between the backend and your AngularJS application
...
## [](#a-brief-introduction-to-angularjs)A brief introduction to AngularJS
...
```html
Copy<tr ng-repeat="user in users">
	<td>{{user.username}}</td>
	<td>{{user.role}}</td>
</tr>
...
...
## [](#angularjs-templates)AngularJS Templates
...
```html
Copy<div class="row thumbnail-wrapper">
...
## [](#scopes-in-angularjs)Scopes in AngularJS
...
## [](#directives-in-angularjs)Directives in AngularJS
