---
title: What\'s new in Spring Data Hopper?
source: https://spring.io/blog/2016/05/03/what-s-new-in-spring-data-hopper
scraped: 2026-02-23T19:17:23.721Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Christoph Strobl |  May 03, 2016 | 7 Comments
---

# What's new in Spring Data Hopper?

_Engineering | Christoph Strobl |  May 03, 2016 | 7 Comments_

As we've just [shipped the GA release](https://spring.io/blog/2016/04/07/spring-data-release-train-hopper-goes-ga-and-sr1) of Spring Data release train [Hopper](https://en.wikipedia.org/wiki/Grace_Hopper), let's take a deeper look at the changes and features that come with the 13 modules on the train. A very fundamental change in the release train's dependencies is the upgrade to Spring Framework 4.2 (currently 4.2.5) as baseline. This is in preparation for the upcoming 4.3 release of the framework. We also took the chance to upgrade our Querydsl integration to 4.x (currently 4.1) which required some breaking changes in very core abstractions. Besides that, Hopper contains quite a few significant major version changes its modules.
...
-   An optional `ExampleMatcher` which carries details and strategies on how to match particular fields, `null`\-values, `String`s in general etc.
-   An `Example`, which consists of the probe and the `ExampleMatcher`.
-   The `QueryByExampleExecutor` interface which your repository would additionally implement and which provides methods taking `Example`s similarly to the `QueryDslPredicateExecutor`.
...
```java
Copyinterface PersonRepository extends CrudRepository<Person, Long>,
...
```java
CopyExampleMatcher exampleSpec = new ExampleMatcher()
...
There are more options available. So please have a look at the reference documentation for Spring Data [JPA](http://docs.spring.io/spring-data/jpa/docs/1.10.x/reference/html/#query-by-example) and [MongoDB](http://docs.spring.io/spring-data/mongodb/docs/1.9.x/reference/html/#query-by-example).

## [](#projections-on-repository-query-methods)Projections on repository query methods

The concept of projections Spring Data REST shipped a feature called projections with the [Evans release](https://spring.io/blog/2014/09/09/spring-data-release-train-evans-goes-ga) train. With Hopper we added support to JPA and MongoDB query methods to use projections on the repository level. A projection is a customized view of your domain model, in this case, returned from a query method.
...
```java
Copyinterface FullNameAndCountry {
...
Please refer to the [Spring Data JPA](http://docs.spring.io/spring-data/jpa/docs/1.10.1.RELEASE/reference/html/#projections) and [Spring Data MongoDB](http://docs.spring.io/spring-data/mongodb/docs/1.9.1.RELEASE/reference/html/#projections) reference documentation for more details in depth how to use projections with query methods.
...
More information and a complete sample how to setup Spring Data Redis to work with a cluster can be found in the [Redis Cluster module](https://github.com/spring-projects/spring-data-examples/tree/master/redis/cluster) of the Spring Data Examples GitHub repository as well as the [reference documentation](http://docs.spring.io/spring-data/redis/docs/current/reference/html/#cluster).
...
As you can see we use method handles here to define both the mapping step back and forth to be picked up by the infrastructure. Of course, there's an alternative overload for the method shown here to work on Java 6 as well. For details on this, make sure you check out the [example dedicated to this on GitHub](https://github.com/spring-projects/spring-data-examples/tree/master/rest/uri-customization).
...
## [](#composable-annotations)Composable annotations
...
Or just have a little fun and translate existing annotations into your [favorite language](https://github.com/sbrannen/spring-polyglot).

The Spring Data team is coming to Las Vegas! Be sure to join us for a lot of sessions on Spring Data at this year's SpringOne Platform. The first batch of featured talks has already been [published](http://springoneplatform.io/sessions) on the event's website. Be sure to [register](https://2016.event.springoneplatform.io/register) to learn about the latest and greatest in Spring Data, the Spring Framework ecosystem in general as well as everything related to CloudFoundry.