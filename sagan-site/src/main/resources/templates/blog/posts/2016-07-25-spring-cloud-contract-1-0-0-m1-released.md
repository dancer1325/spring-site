...
source: https://spring.io/blog/2016/07/25/spring-cloud-contract-1-0-0-m1-released
scraped: 2026-02-23T19:09:47.645Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Marcin Grzejszczak |  July 25, 2016 | 0 Comments
---

# Spring Cloud Contract 1.0.0.M1 Released

_Engineering | Marcin Grzejszczak |  July 25, 2016 | 0 Comments_

On behalf of the Spring Cloud team it is my pleasure to announce the 1.0.0.M1 release of the new Spring Cloud project called Spring Cloud Contract. You can grab it from the [Spring’s milestone repository](https://repo.spring.io/milestone/) or even better - go to [start.spring.io](http://start.spring.io) and pick it from there.

## [](#spring-cloud-contract)[](#spring-cloud-contract)Spring Cloud Contract
...
This article is a companion of another recent one on [how to do zero-downtime deployment with a database](https://spring.io/blog/2016/05/31/zero-downtime-deployment-with-a-database).

## [](#service-providers-and-consumers)[](#service-providers-and-consumers)Service providers and consumers
...
In this blog post we’ll present the approach called "Consumer Driven Contracts" together with the new Spring Cloud project called [Spring Cloud Contract Verifier](https://cloud.spring.io/spring-cloud-contract/spring-cloud-contract.html#_spring_cloud_contract_verifier) formerly known as Accurest and hosted by the [Codearte company](http://codearte.io).

## [](#meanwhile-in-a-company)[](#meanwhile-in-a-company)Meanwhile in a company…​
...
## [](#what-is-consumer-driven-contracts)[](#what-is-consumer-driven-contracts)What is Consumer Driven Contracts?
...
[Spring Cloud Contract Verifier](http://cloud.spring.io/spring-cloud-contract/spring-cloud-contract.html#_spring_cloud_contract_verifier) solves this problem by providing automated solutions to ensure the quality and reliability of the created contracts and their stubs. It consists of the following main features:
...
-   Spring Cloud Contract Verifier Maven / Gradle plugins give you the functionality of converting the Contract into tests and WireMock stubs ([WireMock is a HTTP server stub](http://wiremock.org))
...
## [](#cdc-with-spring-cloud-contract-verifier)[](#cdc-with-spring-cloud-contract-verifier)CDC with Spring Cloud Contract Verifier
...
The [producer code is available here](https://github.com/spring-cloud/spring-cloud-contract/tree/master/samples/standalone/dsl/http-server) and [consumer code here](https://github.com/spring-cloud/spring-cloud-contract/tree/master/samples/standalone/dsl/http-client).

### [](#consumer-side-loan-issuance)[](#consumer-side-loan-issuance)Consumer side (Loan Issuance)
...
```
CopyResponseEntity<FraudServiceResponse> response =
...
```
Copygit clone https://your-git-server.com/server.git local-http-server-repo
```
...
```
Copy[INFO] --- spring-cloud-contract-maven-plugin:1.0.0.BUILD-SNAPSHOT:generateStubs (default-generateStubs) @ http-server ---
...
```
Copy[INFO] Installing /some/path/http-server/target/http-server-0.0.1-SNAPSHOT-stubs.jar to /path/to/your/.m2/repository/com/example/http-server/0.0.1-SNAPSHOT/http-server-0.0.1-SNAPSHOT-stubs.jar
```
...
```
Copy2016-07-19 14:22:25.403  INFO 41050 --- [           main] o.s.c.c.stubrunner.AetherStubDownloader  : Desired version is + - will try to resolve the latest version
...
### [](#producer-side-fraud-detection-server)[](#producer-side-fraud-detection-server)Producer side (Fraud Detection server)
...
```
Copygit checkout -b contract-change-pr master
...
That’s because all the generated tests will extend that class. Over there you can set up your Spring Context or whatever is necessary. In our case we’re using [Rest Assured MVC](http://rest-assured.io/) to start the producer side `FraudDetectionController`.

```
Copypackage com.example.fraud;
...
### [](#consumer-side-loan-issuance-final-step)[](#consumer-side-loan-issuance-final-step)Consumer side (Loan Issuance) final step
...
-   [Consumer Driven Contracts by Ian Robinson](http://martinfowler.com/articles/consumerDrivenContracts.html)
    
-   [Spring Cloud Contract repository](https://github.com/spring-cloud/spring-cloud-contract/)
    
-   [Spring Cloud Contract documentation](https://cloud.spring.io/spring-cloud-contract/spring-cloud-contract.html)
    
-   [Spring Cloud Contract Verifier documentation](https://cloud.spring.io/spring-cloud-contract/spring-cloud-contract.html#_spring_cloud_contract_verifier)
    
-   [Spring Cloud Contract Verifier Maven Plugin documentation](https://cloud.spring.io/spring-cloud-contract/spring-cloud-contract-maven-plugin/)
    
-   [Spring Cloud Contract WireMock documentation](https://cloud.spring.io/spring-cloud-contract/spring-cloud-contract.html#_spring_cloud_contract_wiremock)