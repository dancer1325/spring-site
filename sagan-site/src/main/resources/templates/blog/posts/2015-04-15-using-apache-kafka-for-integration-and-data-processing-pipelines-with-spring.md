---
title: Using Apache Kafka for Integration and Data Processing Pipelines with Spring
source: https://spring.io/blog/2015/04/15/using-apache-kafka-for-integration-and-data-processing-pipelines-with-spring
scraped: 2026-02-23T21:06:23.864Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Josh Long |  April 15, 2015 | 38 Comments
---

# Using Apache Kafka for Integration and Data Processing Pipelines with Spring

_Engineering | Josh Long |  April 15, 2015 | 38 Comments_
...
```
Copy$KAFKA_HOME/bin/zookeeper-server-start.sh  $KAFKA_HOME/config/zookeeper.properties
...
```
Copy$KAFKA_HOME/bin/kafka-server-start.sh  $KAFKA_HOME/config/server.properties
```
...
Like [Spring XD](http://spring.io/projects/spring-xd) and numerous other distributed system, Apache Kafka uses Apache Zookeeper to coordinate cluster information. Apache Zookeeper provides a shared hierarchical namespace (called *znodes*) that nodes can share to understand cluster topology and availability (yet another reason that [Spring Cloud](https://github.com/spring-cloud/spring-cloud-zookeeper) has forthcoming support for it..).
...
## [](#using-apache-kafka-with-spring-integration)Using Apache Kafka with Spring Integration

The recently released [Spring Integration for Apache Kafka 1.1](https://spring.io/blog/2015/03/26/spring-integration-kafka-support-1-1-ga-is-available) is very powerful, and provides inbound adapters for working with both the lower level Apache Kafka API as well as the higher level API.
...
-   org.apache.kafka:kafka\_2.10:0.8.1.1
...
### [](#using-the-spring-integration-apache-kafka-with-the-spring-integration-xml-dsl)Using the Spring Integration Apache Kafka with the Spring Integration XML DSL
...
### [](#using-the-new--apache-kafka-spring-integration-java-configuration-dsl)Using the New Apache Kafka Spring Integration Java Configuration DSL
...
## [](#using-apache-kafka-with-spring-xd)Using Apache Kafka with Spring XD

Apache Kafka is a message bus and it can be very powerful when used as an integration bus. However, it really comes into its own because it's fast enough and scalable enough that it can be used to route big-data through processing pipelines. And if you're doing data processing, you really want [Spring XD](http://projects.spring.io/spring-xd/)! Spring XD makes it dead simple to use Apache Kafka (as the support is built on the Apache Kafka Spring Integration adapter!) in complex stream-processing pipelines. Apache Kafka is exposed as a Spring XD *source* - where data comes from - and a sink - where data goes to.
...
```bash
Copyxd> stream create kafka-source-test --definition "kafka --zkconnect=localhost:2181 --topic=event-stream | log" --deploy

```

And that's it! Naturally, this is just a tase of Spring XD, but hopefully you'll agree the possibilities are tantalizing.

## [](#deploying-a-kafka-server-with-lattice-and-docker)Deploying a Kafka Server with Lattice and Docker
...
```bash
Copyltc create --run-as-root m-kafka spotify/kafka
```
