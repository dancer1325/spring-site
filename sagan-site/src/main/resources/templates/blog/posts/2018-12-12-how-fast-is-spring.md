...
source: https://spring.io/blog/2018/12/12/how-fast-is-spring
scraped: 2026-02-23T15:00:54.549Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Dave Syer |  December 12, 2018 | 8 Comments
---

# How Fast is Spring?

_Engineering | Dave Syer |  December 12, 2018 | 8 Comments_
...
![heap-size-2.1.x](https://docs.google.com/spreadsheets/d/e/2PACX-1vQpSEfx0Y1W9aD3XVyn91-S0jtUp2DRCQSy_W_LMGyMR91YLAQ1mL7MiR1BRd8VzshvtuxzL6WAnlxf/pubchart?oid=336485057&format=image)
...
-   [https://github.com/dsyer/spring-boot-startup-bench](https://github.com/dsyer/spring-boot-startup-bench) - older benchmarks (back to Spring Boot 1.3), fat jar data
    
-   [/static](https://github.com/dsyer/spring-boot-startup-bench/blob/master/static/README.adoc) benchmarks in the same repo - newer, explores classes loaded correlation
    
-   [/flux](https://github.com/dsyer/spring-boot-startup-bench/blob/master/flux/README.adoc) benchmarks in the same repo - WebFlux
    
-   [https://spring.io/blog/2018/10/22/functional-bean-registrations-in-spring-cloud-function](https://spring.io/blog/2018/10/22/functional-bean-registrations-in-spring-cloud-function) - blog on functional beans in Spring Cloud Function
    
-   Spring Fu: [https://github.com/spring-projects/spring-fu](https://github.com/spring-projects/spring-fu)
    
-   [https://github.com/dsyer/spring-boot-allocations](https://github.com/dsyer/spring-boot-allocations) - benchmarks for functional beans and GC pressure
    
-   [https://github.com/dsyer/spring-boot-micro-apps](https://github.com/dsyer/spring-boot-micro-apps) - functional beans and AOT (same code as the "allocations" project but sample apps not benchmarks)
    

## [](#tldr-how-do-i-make-my-app-go-faster)[](#tldr-how-do-i-make-my-app-go-faster)TL;DR How do I make my app go faster?

(Copied from [here](https://github.com/dsyer/spring-boot-allocations).) You are mostly going to have to drop features, so not all of these suggestions will be possible for all apps. Some are not so painful, and actually pretty natural in a container, e.g. if you are building a docker image it’s better to unpack the jar and put application classes in a different filesystem layer anyway.
...
-   Fix the location of the [Spring Boot config file(s)](https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#boot-features-external-config-application-property-files) with `spring.config.location` (command line argument or System property etc.). Example for testing in IDE: `spring.config.location=file://./src/main/resources/application.properties`.
    
-   Switch off JMX if you don’t need it with `spring.jmx.enabled=false` (this is the default in Spring Boot 2.2)
    
-   Make bean definitions lazy by default. There’s a new flag `spring.main.lazy-initialization=true` in Spring Boot 2.2 (and there’s a `LazyInitBeanFactoryPostProcessor` in [this project](https://github.com/dsyer/spring-boot-allocations) you can copy).
...
## [](#some-basic-benchmarks)[](#some-basic-benchmarks)Some Basic Benchmarks

Here is a subset of the [static benchmarks](https://github.com/dsyer/spring-boot-startup-bench/blob/master/static/README.adoc) from [https://github.com/dsyer/spring-boot-startup-bench](https://github.com/dsyer/spring-boot-startup-bench). Each app is started with a new JVM (separate process) per application startup, and has an explicit classpath (not fat jar). The app is always the same, but with different levels of automatic (and in some cases manual) configuration. The "Score" is startup time in seconds, measured as the time from starting the JVM to seeing a marker in the logger output (at this point the app is up and accepting HTTP connections).

```
CopyBenchmark   (sample) Mode  Cnt  Score   Error  Units Beans Classes
...
![pubchart?oid=976086548&format=image](https://docs.google.com/spreadsheets/d/e/2PACX-1vR8B4l5WkWf-9gZWmIYTkmBWM7YWf5bRg852OakrV0G2-vtfM_UkVNRC3cTVk1079HagnMVHYZnvbib/pubchart?oid=976086548&format=image)
...
## [](#garbage-collection-pressure)[](#garbage-collection-pressure)Garbage Collection Pressure
...
![flame_20](https://github.com/dsyer/spring-boot-allocations/raw/master/images/flame_20.svg?sanitize=true)

![flame_21](https://github.com/dsyer/spring-boot-allocations/raw/master/images/flame_21.svg?sanitize=true)

Spring Boot 2.0

Spring Boot 2.1

The red/brown GC flame on the right is noticeably smaller in Spring Boot 2.1. This is a sign of less GC pressure as a result of a [change](https://github.com/spring-projects/spring-framework/commit/81cb740e) in the bean factory internals. The Spring Framework issue behind one of the main changes is [here](https://jira.spring.io/browse/SPR-16918) if you want to look at the details.

Recognizing that GC pressure is an issue is one thing (and async-profiler is the best tool we have found), but locating its source is something of an art. The best tool we have found for that is Flight Recorder (or Java Mission Control) which is part of the OpenJDK release, although it used to be only in the Oracle distribution. The problem with Flight Recorder is that the sampling rate is not really high enough to capture enough data on startup, so you have to try and build tight loops that do something you are interested in, or suspect might be contributing to the problem, and analyse those over a longer period (a few seconds or more). This leads to additional insight, but no real data on whether a "real" application will benefit from changing the hotspot. Much of the code in the [spring-boot-allocations](https://github.com/dsyer/spring-boot-allocations) project is this kind of code: main methods that run tight loops focusing on suspected hotspots that can then be analyzed with Flight Controller.

## [](#webflux-and-micro-apps)[](#webflux-and-micro-apps)WebFlux and Micro Apps

We might expect some variations between apps using a Servlet container and those using the newer reactive runtime from Netty introduced in Spring 5.0. The benchmark figures above are using Tomcat. There are some similar measurements in a different subdirectory of the same repo. Here are the results from the [flux benchmarks](https://github.com/dsyer/spring-boot-startup-bench/blob/master/flux/README.adoc):

```
CopyBenchmark            (sample)  Mode  Cnt  Score   Error  Units Classes
...
## [](#classpath-exclusions)[](#classpath-exclusions)Classpath Exclusions
...
## [](#spring-tweaks)[](#spring-tweaks)Spring Tweaks
...
-   Make bean definitions lazy by default. There’s nothing in Spring Boot that does this, but there’s a `LazyInitBeanFactoryPostProcessor` in [this project](https://github.com/dsyer/spring-boot-allocations) you can copy. It is just a `BeanFactoryPostProcessor` that switches all beans to `lazy=true`.
...
## [](#jvm-tweaks)[](#jvm-tweaks)JVM Tweaks
...
### [](#class-data-sharing)[](#class-data-sharing)Class Data Sharing

Class Data Sharing (CDS) was a commercial only feature of the Oracle JDK since version 7, but it has also available in [OpenJ9](https://www.eclipse.org/openj9/) (the open source version of the IBM JVM) and now in OpenJDK since version 10. OpenJ9 has had CDS for a long time, and it is super easy to use in that platform. It was designed for optimizing memory usage, not startup time, but those two concerns are not unrelated.
...
## [](#functional-bean-definitions)[](#functional-bean-definitions)Functional Bean Definitions
...
```
Copypublic class SampleConfiguration
...
This programming model is not yet mainstream in Spring Boot applications, but it has been implemented in [Spring Cloud Function](https://spring.io/blog/2018/10/22/functional-bean-registrations-in-spring-cloud-function) and is also a basic building block in [Spring Fu](https://spring.io/blog/2018/10/02/the-evolution-of-spring-fu). Also the fastest full Spring Boot benchmark apps above ("bunc") are implemented this way. The main reason for this is that functional bean registration is the fastest way for Spring to create bean instances - it requires virtually no computation beyond instantiating a class and calling its constructors natively.
