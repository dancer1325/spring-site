---
title: Observability with Spring Boot 3
source: https://spring.io/blog/2022/10/12/observability-with-spring-boot-3
scraped: 2026-02-23T09:58:56.535Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Marcin Grzejszczak |  October 12, 2022 | 45 Comments
---

# Observability with Spring Boot 3

_Engineering | Marcin Grzejszczak |  October 12, 2022 | 45 Comments_
...
What is observability? In our understanding, it is *"how well you can understand the internals of your system by examining its outputs"*. We believe that the interconnection between metrics, logging, and distributed tracing gives you the ability to reason about the state of your system in order to debug exceptions and latency in your applications. You can watch more about what we think observability is in [this episode of Enlightning with Jonatan Ivanov](https://tanzu.vmware.com/developer/tv/enlightning/10/).

The upcoming Spring Boot `3.0.0-RC1` release will contain numerous autoconfigurations for improved metrics with [Micrometer](https://micrometer.io/) and new distributed tracing support with [Micrometer Tracing](https://micrometer.io/docs/tracing) (formerly [Spring Cloud Sleuth](https://spring.io/projects/spring-cloud-sleuth)). The most notable changes are that it will contain built-in support for log correlation, [W3C context propagation](https://www.w3.org/TR/trace-context/) will be the default propagation type, and we will support automatic propagation of metadata to be used by the tracing infrastructure (called "remote baggage") that helps to label the observations.
...
# [](#how-does-micrometer-observation-work)[](#how-does-micrometer-observation-work)How Does Micrometer Observation Work?
...
```
Copy// Create an ObservationRegistry
...
# [](#building-your-first-observed-application)[](#building-your-first-observed-application)Building Your First Observed Application

The easiest way to get started is to create a new project from [https://start.spring.io](https://start.spring.io). Make sure to select Spring Boot 3.0.0-SNAPSHOT (you can switch to RC1 once [it’s available](https://calendar.spring.io/)) and your favorite build tool.
...
## [](#webmvc-server-setup)[](#webmvc-server-setup)WebMvc Server Setup
...
To add observation features to your application, choose `spring-boot-starter-actuator` (to add [Micrometer](https://micrometer.io) to the classpath).
...
-   **Tracing**
    ...
    -   For **Tracing Context Propagation** with Micrometer Tracing, we need to pick a **tracer** bridge (**tracer** is a library that is used to handle the lifecycle of a span). We pick [Zipkin Brave](https://zipkin.io) by adding the `io.micrometer:micrometer-tracing-bridge-brave`.
...
-   **Logs**
    ...
    -   Since we have Micrometer Tracing on the classpath, the logs are automatically correlated (that is, they contain a unique trace identifier). Now we need to ship the logs. For this demo, we ship them to [Grafana Loki](https://grafana.com/oss/loki/). We can achieve that by adding the `com.github.loki4j:loki-logback-appender` dependency (check [this link](https://search.maven.org/artifact/com.github.loki4j/loki-logback-appender) for the latest release version)
...
Since we are running the [Grafana](https://grafana.com/grafana/) stack with [Loki](https://grafana.com/oss/loki/) and [Tempo](https://grafana.com/oss/tempo/) locally, we configure the `loki-logback-appender` to send logs to the local instance of Loki.
...
## [](#webmvc-server-code)[](#webmvc-server-code)WebMvc Server Code
...
```
Copy// Example of plugging in a custom handler that in this case will print a statement before and after all observations take place
@Component
...
## [](#resttemplate-client-application-setup)[](#resttemplate-client-application-setup)RestTemplate Client Application Setup
...
-   **Tracing**
    ...
    -   For **Tracing Context Propagation** with Micrometer Tracing, we need to pick a **tracer** bridge. We pick [OpenTelemetry](https://opentelemetry.io) by adding the `io.micrometer:micrometer-tracing-bridge-otel`.
...
-   **Logs**
    ...
    -   As previously, we add the `com.github.loki4j:loki-logback-appender` dependency (check [this link](https://search.maven.org/artifact/com.github.loki4j/loki-logback-appender) for the latest release version) to ship logs to Loki.
...
## [](#resttemplate-application-client-code)[](#resttemplate-application-client-code)RestTemplate Application Client Code
...
## [](#running-it-all-together)[](#running-it-all-together)Running It All Together

We have prepared a Docker setup of the whole observability infrastructure under [this link](https://github.com/marcingrzejszczak/observability-boot-blog-post). Follow these steps to run the infrastructure and both applications.

### [](#running-the-samples)[](#running-the-samples)Running the samples
...
1.  Start up the observability stack (for demonstration purposes, you can use the provided Grafana, Tempo, and Loki stack) and wait for it to start.
    ...
    -   To access Prometheus go to [http://localhost:9090/](http://localhost:9090/)
        
    -   To access Grafana go to [http://localhost:3000/](http://localhost:3000/)
        
2.  Run the server side application (this will block your current terminal window).
    
    ```
    Copy$ ./mvnw spring-boot:run -pl :server
    ```
    
3.  Run the client side application (this will block your current terminal window)
    
    ```
    Copy$ ./mvnw spring-boot:run -pl :client
    ```
    
    You should see log statements similar to these:
    
    ```
    Copy2022-10-04T15:04:55.345+02:00  INFO [client,bbe3aea006077640b66d40f3e62f04b9,93b7a150b7e293ef] 92556 --- [           main] com.example.client.ClientApplication     : Will send a request to the server
...
4.  Go to [Grafana](http://localhost:3000/), go to dashboards, and click on the `Logs, Traces, Metrics` dashboard. There you can pick a trace ID value (for example, `bbe3aea006077640b66d40f3e62f04b9`) to find all logs and traces from both applications that correspond to that trace ID. You should see a following correlated view of logs and traces related to the same trace identifier, and you will see metrics taking place at the same time range. The metrics are related to HTTP request processing latency. These come from the automated Spring Boot WebMvc instrumentation that uses the Micrometer API.
    ...
    Notice a diamond shape in the metrics. These are [`Exemplars`](https://grafana.com/docs/grafana/latest/basics/exemplars/). Those are “specific trace representative of measurement taken in a given time interval”. If you click on the shape, you can jump to the trace ID view to see the corresponding trace.
...
## [](#running-it-all-together-with-aot-support)[](#running-it-all-together-with-aot-support)Running It All Together with AOT Support

To better understand how Spring Boot supports Native, please read [this excellent blog post](https://spring.io/blog/2022/09/26/native-support-in-spring-boot-3-0-0-m5). We reuse that knowledge to run the previously created applications using Spring Native.
...
See also [GraalVM Quickstart](https://www.graalvm.org/java/quickstart/).
...
```
Copy2022-10-10T12:57:07.200+02:00  INFO \[server,,\] 81760 --- \[           main\] com.example.server.ServerApplication     : Starting ServerApplication using Java 17.0.4 on marcin-precision5560 with PID 81760 (/home/marcin/repo/observability/blogs/bootRc1/server/target/server started by marcin in /home/marcin/repo/observability/blogs/bootRc1)
...
### [](#native-support-limitations)[](#native-support-limitations)Native Support Limitations

On the client side, we need to provide the `reflect-config.js` configuration manually. For more information, see [this PR](https://github.com/open-telemetry/opentelemetry-java/pull/4832).
...
Work on the Micrometer Observability would not be possible without the extensive support of the whole Spring team, [Tadaya Tsuyukubo](https://github.com/ttddyy/), [Johnny Lim](https://github.com/izeye), and all the other contributors and reviewers.

# [](#next-steps)[](#next-steps)Next Steps

Based on community feedback, we will continue to improve our Observability story. We intend to go [GA in November this year](https://github.com/micrometer-metrics/micrometer/milestone/177).

This is an exciting time for us. We would again like to thank everyone who has already contributed and reported feedback, and we look forward to further feedback! Check out Spring Boot’s latest snapshots! Check out the documentation of our projects: [Micrometer Context Propagation](https://micrometer.io/docs/contextPropagation), [Micrometer](https://micrometer.io/docs), [Micrometer Observation](https://micrometer.io/docs/observation), [Micrometer Tracing](https://micrometer.io/docs/tracing) and [Micrometer Docs Generator](https://micrometer.io/docs/observation#_documentation_building)! Click [here](https://github.com/marcingrzejszczak/observability-boot-blog-post) to see the code used for this blog post.