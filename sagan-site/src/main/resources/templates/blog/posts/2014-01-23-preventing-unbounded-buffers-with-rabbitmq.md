...
## Per-Queue Message TTL[​](#per-queue-message-ttl "Direct link to Per-Queue Message TTL")
...
This works great when you have data that is only relevant to producers if it arrives on time. *If your data can't be dropped, but you still want the queues to remain as empty as possible, then see below, the section on "dead lettering"*.
...
## Per-Message TTL[​](#per-message-ttl "Direct link to Per-Message TTL")
...
## Queue TTL[​](#queue-ttl "Direct link to Queue TTL")
...
```
Map<String, Object> args = new HashMap<String, Object>();args.put("x-expires", 900000);channel.queueDeclare("myqueue", false, false, false, args);
```
...
## Queue Length Limit[​](#queue-length-limit "Direct link to Queue Length Limit")
...
## Mixing policies[​](#mixing-policies "Direct link to Mixing policies")
...
```
rabbitmqctl set_policy capped_queues "^capped\." \   '{"max-length":10, "expires":900000, "message-ttl":60000}' --apply-to queues
```

## No Queueing at all[​](#no-queueing-at-all "Direct link to No Queueing at all")
...
## Dead Lettering[​](#dead-lettering "Direct link to Dead Lettering")
...
One of the basic questions of queueing theory with regard to requests arriving to our system can be stated as follows[1](#user-content-fn-1-783b80):
...
1.  [Performance Modeling and Design of Computer Systems: Queueing Theory in Action](https://www.amazon.com/Performance-Modeling-Design-Computer-Systems/dp/1107027500) [↩](#user-content-fnref-1-783b80)