---
title: Simple Event Driven Microservices with Spring Cloud Stream
source: https://spring.io/blog/2019/10/15/simple-event-driven-microservices-with-spring-cloud-stream
scraped: 2026-02-23T14:31:40.108Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Ben Wilcock |  October 15, 2019 | 7 Comments
---

# Simple Event Driven Microservices with Spring Cloud Stream

_Engineering | Ben Wilcock |  October 15, 2019 | 7 Comments_

Event driven architecture is great. But without a framework, writing the scaffolding required to work with popular event messaging platforms can be messy. In this post we'll take a look at how [Spring Cloud Stream](https://spring.io/projects/spring-cloud-stream) can be used to simplify your code.
...
Let a flexible messaging abstraction take care of the complex messaging platform integration so you can concentrate on writing simple clean business logic. [Spring Cloud Stream](https://docs.spring.io/spring-cloud-stream/docs/current/reference/htmlsingle/) is a great candidate. It unifies lots of popular messaging platforms behind one easy to use API including RabbitMQ, Apache Kafka, Amazon Kinesis, Google PubSub, Solace PubSub+, Azure Event Hubs, and Apache RocketMQ. It even smoothes away any subtle differences in approach and features between these platforms (like partitioning or exchanges for example) leaving you free to create innovative event-driven solutions.

In the demo that follows, you'll see exactly how Spring Cloud Stream's clever abstractions help make event streaming code cleaner and easier to work with. You'll also see how trivial it is to switch between two different messaging platforms ([RabbitMQ](https://www.rabbitmq.com/) or [Kafka](https://kafka.apache.org/)) using Spring Cloud Stream's `binding` libraries.

## [](#before-you-start)Before you start

These event driven microservices need the latest of these applications installed on your PC[1](#fn-1):

1.  [Java 8](https://adoptopenjdk.net/)
2.  [Docker](https://docs.docker.com/docker-for-mac/install/)(where we'll run RabbitMQ and Kafka locally)
3.  [Git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git)(optional)
4.  Bash (assumed, although alternatives could work)

## [](#running-the-demo)Running The Demo

First, clone the code repository from GitHub. To do this (if you have Git installed) open a new terminal window and issue the following command. If you don't have Git installed, download and extract [this zip file](https://github.com/benwilcock/spring-cloud-stream-demo/archive/master.zip).

```bash
Copygit clone https://github.com/benwilcock/spring-cloud-stream-demo.git
```
...
### [](#step-1-start-the-messaging-servers)Step 1: Start the Messaging Servers

In a fresh terminal window, go to the root folder of the project and issue the following command.

> You'll need ["Docker"](https://docs.docker.com/docker-for-mac/install/) to be installed and running on your system for this script to work properly as it requires `docker-compose`.

```bash
Copy./start-servers.sh
```

This script will start [Kafka](https://kafka.apache.org/) and [RabbitMQ](https://www.rabbitmq.com/) and stream the log output from both to the terminal window (unless you exit with `Ctrl-C`). The servers do not stop when you press `Ctrl-C` - they'll keep running in the background. Once started these servers will all be available to applications running on your computer.

### [](#step-2-choose-between-kafka-or-rabbitmq-mode)Step 2: Choose Between Kafka or RabbitMQ Mode
...
> Note: This demo is **not** designed to "bridge" messages between Kafka and RabbitMQ, so be sure to choose the same profile name in each of the two applications when you compile and run them. If bridging messaging systems is your goal [see the documentation here](https://cloud.spring.io/spring-cloud-static/spring-cloud-stream/2.2.1.RELEASE/spring-cloud-stream.html#multiple-systems).

### [](#step-3-generate-some-loan-events)Step 3: Generate Some Loan Events
...
```bash
Copy./mvnw clean package spring-boot:run -DskipTests=true -P<profile-choice>
```
...
### [](#step-4-process-the-loan-events)Step 4: Process The Loan Events
...
### [](#step-5-stop-the-demo)Step 5: Stop the Demo
...
#### [](#the-loansource-microservice)The Loansource Microservice

For the Loansource misroservice we're using a new feature from Spring Cloud Stream v2.1 - [Spring Cloud Function support](https://cloud.spring.io/spring-cloud-stream/reference/html/spring-cloud-stream.html#spring_cloud_function). With this new feature, all that's required to get the `LoansourceApplication` microservice to act as a source of `Loan` messages is to declare an `@Bean` method which generates and returns a `Supplier<>`. In this case it's a `Supplier` of type `Loan`. The function method code looks something like this...
...
#### [](#the-loancheck-microservice)The Loancheck Microservice
...
```java
Copy  @StreamListener(LoanProcessor.APPLICATIONS_IN)
...
As you can see, the separation of concerns that you get when using [Spring Cloud Streams](https://spring.io/projects/spring-cloud-stream) is very healthy indeed. There is absolutely zero Kafka or RabbitMQ specific code in either microservice. This allows us to focus on the business logic regardless of the messaging platform and you can easily swap messaging platforms simply by changing the "binder" dependencies in the project's `pom.xml`.

## [](#theres-more)There's More...

You can see the events flowing through the messaging platforms as follows:

-   For **Kafka** the [KafDrop](https://hub.docker.com/r/obsidiandynamics/kafdrop) tool on [`localhost:9000`](http://localhost:9000) may be used to observe the topics and the event messages. There is no login required.
    
-   For **RabbitMQ** the Rabbit Management Console can be found on [`localhost:15672`](http://localhost:15672) may be used to observe the exchanges and the event messages. To login the username is `guest` and the password is also `guest`. To observe the actual message contents, you may need to create a Queue manually and bind it to the desired topic using `#` as your `routing key`.
    

To keep up to date with the latest information on Spring Cloud Stream visit the projects dedicated [project page](https://spring.io/projects/spring-cloud-stream) on the Spring website.

To create your own Spring project from scratch, use the project configurator at [start.spring.io](https://start.spring.io).

If you'd like to go deeper with Spring and pure Kafka check out these great blog posts:

1.  [Gary Russell: Spring for Apache Kafka Deep Dive: Error Handling, Message Conversion and Transaction Support](https://www.confluent.io/blog/spring-for-apache-kafka-deep-dive-part-1-error-handling-message-conversion-transaction-support)
    
2.  [Soby Chacko: Spring for Apache Kafka Deep Dive: Apache Kafka and Spring Cloud Stream](https://www.confluent.io/blog/spring-for-apache-kafka-deep-dive-part-2-apache-kafka-spring-cloud-stream)
...
1.  The microservice code in this repository is written and packaged using [Maven](https://maven.apache.org/), [Spring Boot](https://spring.io/projects/spring-boot), and [Spring Cloud Stream](https://spring.io/projects/spring-cloud-stream). At runtime the code relies on [Kafka](https://kafka.apache.org/), [Zookeeper](https://zookeeper.apache.org/), [RabbitMQ](https://www.rabbitmq.com/), and [KafDrop](https://hub.docker.com/r/obsidiandynamics/kafdrop)(a Docker image by by Obsidian Dynamics). Everything in this list has been provided for you - you don't need to install them.[↩](#fnref-1)