...
source: https://spring.io/blog/2024/05/09/spring-ai-structured-output
scraped: 2026-02-23T08:42:54.221Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Christian Tzolov |  May 09, 2024 | 6 Comments
---

# Spring AI - Structured Output

_Engineering | Christian Tzolov |  May 09, 2024 | 6 Comments_
...
# [](#1-structured-output-api)1\. Structured Output API

The `StructuredOutputConverter` interface is defined as:

```
Copypublic interface StructuredOutputConverter<T> extends Converter<String, T>, FormatProvider {
}
```
...
```
Copypublic interface FormatProvider {
	String getFormat();
}
```
...
```
Copy  Your response should be in JSON format.
...
```
CopyStructuredOutputConverter outputConverter = ...
...
## [](#available-output-converters)Available Output Converters
...
# [](#a-nameusing-converters2-using-convertersa)2\. Using Converters
...
## [](#a-namebean-output-converterbean-output-convertera)Bean Output Converter
...
```
Copyrecord ActorsFilms(String actor, List<String> movies) {
}
```
...
```
CopyActorsFilms actorsFilms = ChatClient.create(chatModel).prompt()
...
#### [](#a-namebean-output-converter-generic-typessupport-generic-bean-typesa)Support Generic Bean Types
...
```
CopyList<ActorsFilms> actorsFilms = ChatClient.create(chatModel).prompt()
...
```
CopyBeanOutputConverter<List<ActorsFilms>> outputConverter = new BeanOutputConverter<>(
...
## [](#a-namemap-output-convertermap-output-convertera)Map Output Converter

Following sniped shows how to use `MapOutputConverter` to generate a list of numbers.

```
CopyMap<String, Object> result = ChatClient.create(chatModel).prompt()
...
```
CopyMapOutputConverter mapOutputConverter = new MapOutputConverter();
...
## [](#a-namelist-output-converterlist-output-convertera)List Output Converter

Following snippet shows how to use `ListOutputConverter` to generate a list of ice cream flavours.

```
CopyList<String> flavors = ChatClient.create(chatModel).prompt()
...
```
CopyListOutputConverter listOutputConverter = new ListOutputConverter(new DefaultConversionService());
