---
title: Spring Boot Memory Performance
source: https://spring.io/blog/2015/12/10/spring-boot-memory-performance
scraped: 2026-02-23T19:33:28.680Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Dave Syer |  December 10, 2015 | 24 Comments
---

# Spring Boot Memory Performance

_Engineering | Dave Syer |  December 10, 2015 | 24 Comments_
...
## [](#vanilla-spring-boot-app)Vanilla Spring Boot App
...
```
Copy$ jar -tvf  target/demo-0.0.1-SNAPSHOT.jar | grep lib/.*.jar | awk '{tot+=$1;} END {print tot}'
18893563
```
...
Here's a graph of the heap usage from a quiescent app under load, followed by a manual garbage collection (the double nick in the middle) and a new equilibrium with a lower heap usage.

![](https://raw.githubusercontent.com/dsyer/spring-boot-memory-blog/master/manual-gc-web.png)

Some tools in the JVM other than JConsole might also be interesting. The first is `jps` which is useful for getting the process id of the app you want to inspect with the other tools:
...
```
Copy$ sudo ~/Programs/jdk1.8.0/bin/jmap -clstats 4330
...
### [](#kernel-memory-tools)Kernel Memory Tools
...
A lower level tool is `pmap`, where we can look at the memory allocations assigned to a process. Numbers from `pmap` don't seem to make much sense either:
...
### [](#scale-up-processes)Scale up Processes
...
```
Copy$ for f in {8080..8119}; do (java -Xmx32m -Xss256k -jar target/demo-0.0.1-SNAPSHOT.jar --server.port=$f 2>&1 > target/$f.log &); done
```
...
## [](#do-nothing-plain-java-app)Do Nothing Plain Java App
...
```
Copypublic class Main throws Exception {
...
## [](#do-nothing-spring-boot-app)Do Nothing Spring Boot App
...
![](https://raw.githubusercontent.com/dsyer/spring-boot-memory-blog/master/manual-gc.png)
...
```
Copy$ ls -l build/distributions/ratpack/lib/*.jar | awk '{tot+=$5;} END {print tot}'
16277607
```
...
## [](#ratpack-java-app)Ratpack Java App

Here's a really basic static app:

```
Copyimport ratpack.server.BaseDir;
...
## [](#variations-on-the-vanilla-app)Variations on the Vanilla App
...
![](https://raw.githubusercontent.com/dsyer/spring-boot-memory-blog/master/manual-gc-tomcat.png)
...
### [](#deploy-another-application)Deploy another application
...
## [](#rule-of-thumb-process-sizes)Rule of Thumb Process Sizes
...
![](https://raw.githubusercontent.com/dsyer/spring-boot-memory-blog/master/non-heap-correlation.png)
...
\* Only the Eureka client has a larger heap: all the others are set explicitly to `-Xmx32m`.
