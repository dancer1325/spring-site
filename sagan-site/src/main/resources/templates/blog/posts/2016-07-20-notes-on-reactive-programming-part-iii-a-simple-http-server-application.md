...
```
Copyprivate RestTemplate restTemplate = new RestTemplate();
...
```
Copyprivate Mono<HttpStatus> fetch(int value) {
...
### [](#embedding-in-a-non-reactive-application)[](#embedding-in-a-non-reactive-application)Embedding in a Non-Reactive Application

If we wanted to run the scatter-gather code above in a non-reactive environment, we could use Spring MVC like this:

```
Copy@RequestMapping("/parallel")
...
### [](#no-such-thing-as-a-free-lunch)[](#no-such-thing-as-a-free-lunch)No Such Thing as a Free Lunch
...
## [](#reactive-all-the-way-down)[](#reactive-all-the-way-down)Reactive all the Way Down
...
```
Copyprivate WebClient client = new WebClient(new ReactorHttpClientRequestFactory());
...
in the [sample code](https://github.com/dsyer/reactive-notes) the "reactive" sample has Maven profiles "tomcat", "tomcatNext" (for Tomcat 8.5), "jetty" and "netty", so you can easily try out all the different server options without changing a line of code.
...
### [](#still-no-free-lunch)[](#still-no-free-lunch)Still No Free Lunch
