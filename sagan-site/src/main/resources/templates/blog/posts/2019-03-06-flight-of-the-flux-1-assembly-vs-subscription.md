---
title: Flight of the Flux 1 - Assembly vs Subscription
source: https://spring.io/blog/2019/03/06/flight-of-the-flux-1-assembly-vs-subscription
scraped: 2026-02-23T14:18:30.654Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Simon Baslé |  March 06, 2019 | 8 Comments
---

# Flight of the Flux 1 - Assembly vs Subscription

_Engineering | Simon Baslé |  March 06, 2019 | 8 Comments_
...
When you first learn about *Reactive Streams* and *reactive programming* on the JVM, the first thing you learn is the high-level relationship between `Publisher` and `Subscriber`: one produces data, the other consumes it. Simple right? Furthermore, it appears that the `Publisher` **pushes** data to the `Subscriber`.
...
```java
CopyMono<HttpResponse> httpSource = makeHttpRequest();
...
```java
CopyMono<String> quote = makeHttpRequest()
...
```java
Copy//you could derive a `Mono<String>` of odd-length strings vs even-length ones
...
So far, we've *assembled an asynchronous pipeline*. That is, we've instantiated `Flux` and `Mono` variables through the use of *operators*, that results other `Flux`/`Mono` with behavior layered like an onion.
...
That signalling of interest is propagated backwards through the chain of operators, up until the *source* operator, the `Publisher` that actually produces the initial data:

```java
CopymakeHttpRequest() //<5>
...
3.  Which extracts the *quote* and passes it to the `flatMapMany`
...
```java
CopyMono<String> evenLength = quote.filter(str -> str.length() % 2 == 0);
Mono<String> oddLength = quote.filter(str -> str.length() % 2 == 1);
Flux<String> words = quote.flatMapMany(quote -> Flux.fromArray(quote.split(" ")));
...
```java
CopyFlux<Long> clockTicks = Flux.interval(Duration.ofSeconds(1));
...
```java
CopyFlux<Long> coldTicks = Flux.interval(Duration.ofSeconds(1));
