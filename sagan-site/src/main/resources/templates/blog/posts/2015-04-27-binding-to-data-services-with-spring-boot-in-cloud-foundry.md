---
title: Binding to Data Services with Spring Boot in Cloud Foundry
source: https://spring.io/blog/2015/04/27/binding-to-data-services-with-spring-boot-in-cloud-foundry
scraped: 2026-02-23T21:04:45.528Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Dave Syer |  April 27, 2015 | 12 Comments
---

# Binding to Data Services with Spring Boot in Cloud Foundry

_Engineering | Dave Syer |  April 27, 2015 | 12 Comments_
...
> NOTE: The current co-ordinates for all the libraries being discussed are `org.springframework.boot:spring-boot-*:2.2.5.RELEASE`, `org.springframework.cloud:spring-cloud-*-connector:1.0.8.RELEASE`, `org.cloudfoundry:auto-reconfiguration:2.11.0.RELEASE`.
...
## [](#punchline-for-the-impatient)Punchline for the Impatient
...
```
Copyspring.datasource.url: ${vcap.services.mysql.credentials.jdbcUrl:jdbc:h2:mem:testdb}
...
Plus any other `spring.datasource.*` properties that you need.

Another choice is to create a `DataSource` explicitly using [Spring Cloud Connectors](http://cloud.spring.io/spring-cloud-connectors), but guarded by the "cloud" profile:
...
## [](#layers-of-autoconfiguration)Layers of Autoconfiguration
...
```
CopyVCAP_APPLICATION={"name":"application","instance_id":"FOO"}
...
-   Add the "auto-reconfiguration" JAR from the Java buildpack (Maven co-ordinates `org.cloudfoundry:auto-reconfiguration:2.11.0.RELEASE`). You can add it as a local dependency to simulate running an application in Cloud Foundry, but it wouldn't be normal to do this with a real application (this is just for experimenting with autoconfiguration). The auto-reconfiguration JAR now has everything it needs to create a `DataSource`, but it doesn't (yet) because it detects that you already have a bean of type `CloudFactory`, one that was added by Spring Boot.
...
```
Copy015-04-14 15:11:11.765  INFO 12727 --- [           main] urceCloudServiceBeanFactoryPostProcessor : Skipping auto-reconfiguring beans of type javax.sql.DataSource
...
### [](#using-cloudfactory-to-create-a-datasource)Using CloudFactory to create a DataSource
...
## [](#dual-running-local-with-h2-in-the-cloud-with-mysql)Dual Running: Local with H2, in the Cloud with MySQL
...
## [](#manually-creating-a-local-and-a-cloud-datasource)Manually Creating a Local and a Cloud DataSource
...
## [](#using-a-non-embedded-database-locally)Using a Non-Embedded Database Locally
...
## [](#a-purely-declarative-approach)A Purely Declarative Approach
...
```properties
Copyspring.datasource.url: ${cloud.services.mysql.connection.jdbcurl:jdbc:h2:mem:testdb}
...
> Note: similar features are provided by Spring Boot, which provides `vcap.services.*` instead of `cloud.services.*`, so you actually end up with more than one way to do this. However, the JDBC urls has slightly different names, so watch out for "jdbcurl" vs. "jdbcUrl". Example:
> 
> ```
> Copyspring.datasource.url: ${vcap.services.mysql.credentials.jdbcUrl:jdbc:h2:mem:testdb}
> spring.datasource.username: ${vcap.services.mysql.credentials.username:sa}
...
> ```
> 
> Usually non-JDBC services work fine with the corresponding `vcap.services.*credentials.url`.
...
> NOTE: If you use `JBP_CONFIG_SPRING_AUTO_RECONFIGURATION` to switch off the buildpack autoreconfiguration, you also switch off the "cloud" profile. It's easy to add it back of course.

## [](#mixed-declarative-and-explicit-bean-definition)Mixed Declarative and Explicit Bean Definition
...
## [](#a-third-way-discover-the-credentials-and-bind-manually)A Third Way: Discover the Credentials and Bind Manually
...
## [](#deploying-to-multiple-cloud-platforms)Deploying to Multiple Cloud Platforms
...
## [](#summary-of-autoconfiguration-and-provided-behaviour)Summary of Autoconfiguration and Provided Behaviour
...
-   The Cloud Foundry Java buildpack detects a Spring Boot application and activates the "cloud" profile, unless it is already active. Adding the buildpack auto-reconfiguration JAR does the same thing if you want to try it locally. You can disable it with `JBP_CONFIG_SPRING_AUTO_RECONFIGURATION='{ enabled: false }'`.
...
-   Switching off the Spring Boot `CloudAutoConfiguration` is easy, but if you do that, you have to remember to switch off the buildpack auto-reconfiguration as well if you don't want it. There are 2 ways to do that. One is an env var: `JBP_CONFIG_SPRING_AUTO_RECONFIGURATION='{ enabled: false }'`. The other is to define a bean definition (can be of type `Cloud` or `CloudFactory` for instance).
...
## [](#general-advice-and-conclusion)General Advice and Conclusion
...
2.  Use the Spring Boot default autoconfiguration and declare the cloud bindings using `application.properties` (or in YAML). To take full advantage you have to explicitly switch off the buildpack auto-reconfiguration as well.
