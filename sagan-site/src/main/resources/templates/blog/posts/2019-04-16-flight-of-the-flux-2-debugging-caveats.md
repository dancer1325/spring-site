---
title: Flight of the Flux 2 - Debugging Caveats
source: https://spring.io/blog/2019/04/16/flight-of-the-flux-2-debugging-caveats
scraped: 2026-02-23T14:18:35.048Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Simon Baslé |  April 16, 2019 | 1 Comment
---

# Flight of the Flux 2 - Debugging Caveats

_Engineering | Simon Baslé |  April 16, 2019 | 1 Comment_
...
## [](#debugging-in-a-reactive-world)Debugging in a Reactive World
...
### [](#demonstrating-the-issue-with-vanilla-async-code)Demonstrating the issue with vanilla async code

Let's take an example with an `ExecutorService` and a `Future` (no Reactor code here):

```java
Copy	private static void imperative() throws ExecutionException, InterruptedException {
...
```
Copyjava.util.concurrent.ExecutionException: java.lang.ArrayIndexOutOfBoundsException: Index 5 out of bounds for length 4
...
### [](#demonstrating-the-issue-in-reactor)Demonstrating the issue in Reactor

If we look for a Reactor equivalent to the above code, we can come up with this:

```java
Copy	private static void reactive() {
		int seconds = LocalTime.now().getSecond();
		Mono<Integer> source;
		if (seconds % 2 == 0) {
...
```
Copyjava.lang.IndexOutOfBoundsException
...
```java
Copy	private static void reactiveNoSubscribeOn() {
		int seconds = LocalTime.now().getSecond();
		Mono<Integer> source;
		if (seconds % 2 == 0) {
			source = Flux.range(1, 10)
...
### [](#back-to-classics-log)Back to classics: `log`
...
```
Copy17:01:23.711 [main] INFO  source C - | onSubscribe([Fuseable] MonoElementAt.ElementAtSubscriber)
...
### [](#enriching-stacktraces-with-debug-mode)Enriching stacktraces with Debug Mode
...
### [](#bringing-the-cost-down-with-checkpoint)Bringing the cost down with `checkpoint`
...
```java
Copy	private static void checkpoint() {
		int seconds = LocalTime.now().getSecond();
		Mono<Integer> source;
		if (seconds % 2 == 0) {
			source = Flux.range(1, 10)
