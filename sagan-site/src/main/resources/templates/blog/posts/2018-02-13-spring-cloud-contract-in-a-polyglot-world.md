...
source: https://spring.io/blog/2018/02/13/spring-cloud-contract-in-a-polyglot-world
scraped: 2026-02-23T16:08:55.185Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Marcin Grzejszczak |  February 13, 2018 | 6 Comments
---

# Spring Cloud Contract in a polyglot world

_Engineering | Marcin Grzejszczak |  February 13, 2018 | 6 Comments_
...
# [](#what-is-contract-testing)[](#what-is-contract-testing)What is Contract Testing

In order to increase the certainty that our systems behave properly, we write different types of tests. According to the [test pyramid](https://martinfowler.com/bliki/TestPyramid.html) the main types of tests are unit, integration, and UI. The more complex the tests, the more time and effort they require and the more brittle they become.

In a distributed system, one of the most frequent problems is testing integrations between applications. Let’s assume that your service sends a REST request to another application. When using Spring Boot, you can write a `@SpringBootTest` in which you test that behavior. You set up a Spring context, you prepare a request to be sent…​ and where do you send it? You haven’t started the other application, so you get a `Connection Refused` exception. You can try mocking the real HTTP call and returning a fake response. However, if you do that, you do not test any real HTTP integration, serialization and deserialization mechanisms, and so on. You could also start a fake HTTP server (for example, [WireMock](http://wiremock.org)) and simulate how it should behave. The problem here is that you, as a client of an API, define how the server behaves. In other words, if you tell the fake server to return text `testText` when a request is sent to endpoint `/myEndpoint`, it does just that, even if the real server does not have such an endpoint. In short, the problem is that the stubs might not be reliable.
...
-   In the **Producer Contract** testing approach, the producer defines the contracts and writes the contract tests, describes the API, and publishes the stubs without any cooperation with its clients. Often, this happens when the API is public and the owners of the API don’t even know who exactly is using it. An example is [Spring Initializr](https://start.spring.io), which publishes its stubs with [Spring Rest Docs](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/multi/multi__spring_cloud_contract_wiremock.html#_generating_stubs_using_rest_docs) tests. The stubs for version `0.5.0.BUILD-SNAPSHOT` are available [here with the `stubs` classifier](https://repo.spring.io/libs-snapshot/io/spring/initializr/initializr-web/0.5.0.BUILD-SNAPSHOT/).
...
# [](#introduction-to-maven-nomenclature)[](#introduction-to-maven-nomenclature)Introduction to Maven Nomenclature

Since it’s much easier now to use Spring Cloud Contract for non-JVM projects, it’s good to explain the basic terms behind the packaging defaults and introduce the [Maven](https://maven.apache.org/) nomenclature.

Tip

Apache Maven is a software project management and comprehension tool. Based on the concept of a project object model (POM), Maven can manage a project’s build, reporting, and documentation from a central piece of information. (See [https://maven.apache.org/](https://maven.apache.org/))
...
-   `Artifact manager`: When you generate binaries, sources, or packages, you would like them to be available for others to download, reference, or reuse. In the JVM world, those artifacts would be JARs. For Ruby, these would be gems. For Docker, they would be Docker images. You can store those artifacts in a manager. Examples of such managers include [Artifactory](https://jfrog.com/artifactory/) and [Nexus](http://www.sonatype.org/nexus/).
    

# [](#what-is-spring-cloud-contract)[](#what-is-spring-cloud-contract)What is Spring Cloud Contract

[Spring Cloud Contract](https://cloud.spring.io/spring-cloud-contract/) is an umbrella project holding solutions that help users to implement different sorts of contract tests. It comes with two main modules: `Spring Cloud Contract Verifier`, which is used mainly by the producer side, and `Spring Cloud Contract Stub Runner`, which is used by the consumer side.

The project lets you define contracts using:

-   [Groovy DSL](http://www.groovy-lang.org/)
    
-   [YAML](http://yaml.org/)
    
-   [Pact JSON](https://docs.pact.io/)
    
-   [Spring Rest Docs](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/multi/multi__spring_cloud_contract_wiremock.html#_generating_stubs_using_rest_docs)
    

Let’s assume that we’ve decided to write the contracts using YAML. On the producer side, from the contracts:

-   Tests are generated with a Maven or [Gradle](https://gradle.org/) plugin to assert that the contract is met.
...
-   Uses Stub Runner to fetch the stubs of the producer. Stub Runner starts in memory HTTP servers (by default, those are [WireMock servers](https://wiremock.org)) fed with the stubs.
...
# [](#what-is-the-current-problem-with-spring-cloud-contract)[](#what-is-the-current-problem-with-spring-cloud-contract)What is the Current "Problem" with Spring Cloud Contract
...
# [](#spring-cloud-contract-and-polyglot-support)[](#spring-cloud-contract-and-polyglot-support)Spring Cloud Contract and Polyglot Support
...
In order to hide the implementation details (such as generation of java tests, plugin setup, or Java installation), we needed to introduce a layer of abstraction. We decided to hide those by using [Docker](https://www.docker.com/) images. We encapsulated all the project setup, required packages, and folder structures inside a docker image in such a way that no knowledge, other than required environment variables, is required from the user.

We introduced Docker images for both the [producer](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/single/spring-cloud.html#docker-project) and the [consumer](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/single/spring-cloud.html#stubrunner-docker). All the JVM-related logic gets wrapped in a Docker container, which means that you don’t have to have Java installed to generate tests and run the stubs with Stub Runner.

The following sections go through an example of a NodeJS application being tested by using Spring Cloud Contract. The code was forked from [https://github.com/bradtraversy/bookstore](https://github.com/bradtraversy/bookstore) and is available under [https://github.com/spring-cloud-samples/spring-cloud-contract-nodejs](https://github.com/spring-cloud-samples/spring-cloud-contract-nodejs). Our aim is to start generating tests and stubs for an existing application as quickly as possible with the least effort.

# [](#spring-cloud-contract-on-the-producer-side)[](#spring-cloud-contract-on-the-producer-side)Spring Cloud Contract on the Producer Side

Let’s clone the simple NodeJS MVC application, as follows:

```
Copy$ git clone https://github.com/spring-cloud-samples/spring-cloud-contract-nodejs
...
## [](#spring-cloud-contract-docker-image)[](#spring-cloud-contract-docker-image)Spring Cloud Contract Docker Image

The image is available on [DockerHub under SpringCloud org](https://hub.docker.com/r/springcloud/spring-cloud-contract/).
...
-   Generates the [WireMock](http://wiremock.org) stubs.
...
## [](#spring-cloud-contract-docker-image-setup)[](#spring-cloud-contract-docker-image-setup)Spring Cloud Contract Docker Image setup
...
-   `REPO_WITH_BINARIES_URL` - The URL of your Artifact Manager. Defaults to `[http://localhost:8081/artifactory/libs-release-local](http://localhost:8081/artifactory/libs-release-local)`, which is the default URL of [Artifactory](https://jfrog.com/artifactory/) when running locally.
...
# [](#running-spring-cloud-contract-tests-on-the-producer-side)[](#running-spring-cloud-contract-tests-on-the-producer-side)Running Spring Cloud Contract tests on the Producer Side

Important

To run this sample you need to have [Docker](https://www.docker.com), [Docker Compose](https://docs.docker.com/compose/) and [npm](https://docs.npmjs.com/cli/install) installed.
...
```
Copy# Install the required npm packages
$ npm install
...
# [](#using-spring-cloud-contract-stubs-on-the-consumer-side)[](#using-spring-cloud-contract-stubs-on-the-consumer-side)Using Spring Cloud Contract Stubs on the Consumer Side

In this example, We publish a [spring-cloud/spring-cloud-contract-stub-runner](https://hub.docker.com/r/springcloud/spring-cloud-contract-stub-runner/) Docker image that starts the standalone version of Stub Runner.
...
You can pass any of the [properties](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/single/spring-cloud.html#common-properties-junit-spring) as environment variables. The convention is that all the letters should be upper case and that the word delimiters and the dots (`.`) should be replaced with underscores (`_`). For example, the `stubrunner.repositoryRoot` property should be represented as a `STUBRUNNER_REPOSITORY_ROOT` environment variable.
...
```
Copy# Provide the Spring Cloud Contract Docker version
$ export SC_CONTRACT_DOCKER_VERSION="1.2.3.RELEASE"
...
-   Downloads the stubs from Artifactory at `[http://192.168.0.100:8081/artifactory/libs-release-local](http://192.168.0.100:8081/artifactory/libs-release-local)`.
...
```
Copy# let's execute the first request (no response is returned)
...
-   Read the documentation of [Spring Cloud Contract](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/single/spring-cloud.html#docker-project).
    
-   Read the documentation of [Spring Cloud Contract Stub Runner](https://cloud.spring.io/spring-cloud-static/Edgware.SR2/single/spring-cloud.html#stubrunner-docker).
    
-   Check out the [Bookstore sample](https://github.com/spring-cloud-samples/spring-cloud-contract-nodejs).
    
-   Ask questions on [Spring Cloud Contract Gitter channel](https://gitter.im/spring-cloud/spring-cloud-contract).