---
title: The Joy of Mustache: Server Side Templates for the JVM
source: https://spring.io/blog/2016/11/21/the-joy-of-mustache-server-side-templates-for-the-jvm
scraped: 2026-02-23T18:57:37.929Z
description: Level up your Java code and explore what Spring can do for you.
meta: Engineering | Dave Syer |  November 21, 2016 | 14 Comments
---

# The Joy of Mustache: Server Side Templates for the JVM

_Engineering | Dave Syer |  November 21, 2016 | 14 Comments_

> NOTE: If you are reading this in December 2023 or later, there was a 1.16 release of JMustache that added "inheritance" support. This is an optional [Mustache spec](https://github.com/mustache/spec/blob/master/specs/~inheritance.json) feature but was previously unimplemented in JMustache. It allows you to do the "layout with included body" type templating that is very common in web apps, and needed in the sample for this article. The sample has been updated in Github to use inheritance instead of the workaround with lambdas presented below.

I don’t do much server-side templating, but when I do…​ well frankly, I tend to forget things. Every template language has its strengths and weaknesses, and they all have syntax to remember, and more frequently to forget. Recently I completed some work on the old [Spring Petclinic](https://github.com/spring-projects/spring-petclinic), converting it to use [Thymeleaf](http://www.thymeleaf.org/) in the view layer, and re-organizing the code to be a bit more "modern". I enjoyed working with Thymeleaf 3, and found it a pleasant experience, but had to spend a lot of time scanning documentation and samples. Then I had another little project that needed some templates, and I remembered my fondness for [Mustache](http://mustache.github.com), which we added to Spring Boot back in version 1.2, and which plays an important role in the excellent [Spring REST Docs](https://github.com/spring-projects/spring-restdocs) tool. I added `spring-boot-starter-mustache` to my new project, and was up and running within seconds.

I want to show you what a neat little tool [JMustache](https://github.com/samskivert/jmustache) is for server side rendering of HTML (or anything else in plain text come to that). I always liked [Mustache](http://mustache.github.com/) because of its simplicity - it’s "just enough" templating - and you really couldn’t wish for a cleaner, leaner, more lightweight library than this one, if you have to render templates in the JVM. There is one jar file with no dependencies, and it adds 78kb to your classpath, which isn’t going to hurt anyone, and will put a smile on many faces. It has very few features, which is excellent for people who can’t remember syntax, and the manual is short, comprehensive, readable, and useful.
...
There is some sample code following the text in [GitHub](https://github.com/dsyer/mustache-sample). It is a tiny Spring MVC application, also using Spring Security. If you want to see it develop in stages along with the text, you can use some tags:
...
```
Copy$ git clone https://github.com/dsyer/mustache-sample
$ cd mustache-sample
$ git checkout base
...
The app runs on [http://localhost:8080](http://localhost:8080), and you can authenticate with any username and password (even empty!). There are no real features in the sample app, but it does have login and logout and a home page, to provide some hooks to show the templating features.

## [](#getting-started)[](#getting-started)Getting Started

[Spring Boot](http://projects.spring.io/spring-boot/) has autoconfiguration support for JMustache, so it is easy to get up and running with a Spring MVC application. You could generate a project from the [Spring Initializr](https://start.spring.io), and ask it for `spring-boot-starter-mustache`.
...
### [](#make-the-application-secure)[](#make-the-application-secure)Make the Application Secure
...
### [](#custom-authentication-processing-with-spring-mvc)[](#custom-authentication-processing-with-spring-mvc)Custom Authentication Processing with Spring MVC
...
```
Copyprivate AuthenticationSuccessHandler handler = new SavedRequestAwareAuthenticationSuccessHandler();
...
### [](#the-login-form)[](#the-login-form)The Login Form
...
```
Copyspring.mustache.expose-request-attributes=true
```
...
## [](#layout-abstractions-using-includes)[](#layout-abstractions-using-includes)Layout Abstractions: Using Includes
...
### [](#natural-templates)[](#natural-templates)"Natural" Templates
...
## [](#layout-abstractions-using-a-lambda)[](#layout-abstractions-using-a-lambda)Layout Abstractions: Using a Lambda
...
### [](#rendering-the-layout-in-the-lambda)[](#rendering-the-layout-in-the-lambda)Rendering the Layout in the Lambda
...
```
Copyclass Layout implements Mustache.Lambda {
...
### [](#more-dynamic-content)[](#more-dynamic-content)More Dynamic Content
...
```
Copy{{#layout}}{{#title}}Home Page{{/title}}
    <h1>Demo</h1>
    <div>Hello World</div>\
{{/layout}}
```
...
## [](#logout-menus-and-spring-boot-configuration)[](#logout-menus-and-spring-boot-configuration)Logout: Menus and Spring Boot Configuration
...
```
Copyprivate Application application;
